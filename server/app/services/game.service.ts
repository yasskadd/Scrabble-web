...
export class GameService {
     ...
     */
    play(playerName: string, firstCoordinate: GameboardCoordinate, direction: string, letters: string[]): [boolean, GameBoard] {
        let gameBoard: [boolean, GameBoard] = [false, this.gameboard];
        if (this.turn.validating(playerName) && this.player1.name === playerName) {
            gameBoard = this.letterPlacement.placeLetter(this.player1, firstCoordinate, direction, letters, this.gameboard);
            this.turn.end();
            return gameBoard;
        } else if (this.turn.validating(playerName) && this.player2.name === playerName) {
            gameBoard = this.letterPlacement.placeLetter(this.player2, firstCoordinate, direction, letters, this.gameboard);
            this.turn.end();
            return gameBoard;
        }
